...
for page_no in range(1, 25):
    ...
    for i in range(pos, len(prob)):
        ...
        problem_div = (prob[i].find_all("div", {"class": "mx-2 py-[11px]"}))[
            1
        ].div.div.div
        ...
        if len(problem_div.find_all("svg", {"class": "ml-2 shrink-0"})) != 0:
            continue
            # removing premium problems

        problem_url = "https://leetcode.com" + str(
            (
                (prob[i].find_all("div", {"class": "mx-2 py-[11px]"}))[
                    1
